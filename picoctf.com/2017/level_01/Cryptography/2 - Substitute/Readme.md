...
## Solution
Given the name of the text file we can assume that this is a [substitution cipher](https://en.wikipedia.org/wiki/Substitution_cipher). We could take some time creating a script to crack this by ourselves but why reinvent the wheel? As advised in the hint we will use a tool to solve this problem. This [substitution solver](https://www.guballa.de/substitution-solver) gave the following answer:  
